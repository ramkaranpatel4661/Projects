...
import { Search, Menu, X, User, LogOut, Plus, MessageCircle, Package } from 'lucide-react'
...
=======
    const fetchUnread = async () => {
      try {
        ...
        const count = conversations.reduce((count, chat) => {
          return count + chat.messages.filter(
            msg => !msg.isRead && msg.sender._id !== user._id
          ).length;
        }, 0);
        setUnreadCount(count);
    ...
    if (user) {
      ...
      if (socket) {
        ...
        socket.on('new_message', (data) => {
          console.debug('🔔 Header.jsx received new_message:', data);
...
>>>>>>> d66948e2f811c58f792613f59ccb6fb27553790e
